...
function loadConfig(cwd) {
  ...
  return {
    ...
    deepseek: {
      ...
      model: env.SUPATHINK_DEEPSEEK_MODEL || 'deepseek-v4-flash',
    },
    env,
    project,
  };
...
function detectPanel(text) {
  ...
  if (slash) return /panel-?lite/.test(slash[1]) ? 'lite' : slash[1];
  const mark = t.match(/^\s*SUPATHINK_PANEL=(panel-?lite|panel|lite|debate|delphi|redblue)\b/);
  return mark ? (/panel-?lite|lite/.test(mark[1]) ? 'lite' : mark[1]) : null;
}
...
/** 帮助/发现入口:裸 /st(或 /st:help、/st:?)——TG 菜单唯一合法条目,返回 /st:* 命令清单 */
function detectHelp(text) {
  return /^\s*\/st[:\/-]?(?:help|\?|list|命令)?\s*$/.test(String(text || ''));
}

const ST_COMMANDS = [
  '/st:slow <议题> —— 本轮深度校验(full 档)',
  '/st:slow-full <议题> —— 深审 + 先自出账本',
  '/st:fast —— 本轮跳过校验',
  '/st:on · /st:off —— 本会话开/关自动分档',
  '/st:panel <议题> —— 多模型三席合议 + Judge',
  '/st:debate <议题> —— 正反论辩',
  '/st:delphi <议题> —— 匿名估计合议',
  '/st:redblue <议题> —— 蓝案红攻压测',
  '/st:altitude —— 对齐抬头(目标/代理指标)',
  '/st:win <一句话> —— 记录一次真实救场',
];

module.exports = { ROOT, loadConfig, detectForce, detectSlowFull, detectSessionToggle, detectPanel, detectAltitude, detectWin, detectHelp, ST_COMMANDS, parseEnvFile };
