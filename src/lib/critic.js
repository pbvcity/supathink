...
function runGpt54Mini(draft) {
  ...
  let review = null;
  try {
    const raw = fs.readFileSync(path.join(workDir, `codex-critic-${process.pid}.txt`), 'utf8');
    const outer = extractJson(raw);
    review = outer && outer.verdict
      ? outer
      : extractJson((outer && (outer.content || outer.reasoning_content || outer.reasoning)) || '');
