...
const RE_DECISION = [
  /该不该|要不要|应不应该|是否.{0,8}(值得|适合|划算)|是否.{0,8}应该[^。？！?\n]{0,16}(买|卖|选|做|去|留|投|签|加仓|减仓|清仓|止盈|止损|长持)|应该.{0,8}吗|值不值得|值得.{0,8}(吗|长期|长持|持有)|划算吗/,
  /(持仓|股票|基金|币|仓位)[^。？！?\n]{0,24}(长持|长期持有|加仓|减仓|清仓|止盈|止损)/,
  ...
  /\b(?:my|our)\s+\$?[a-z0-9.]{1,8}\s+position\b.{0,50}\b(?:worth\s+(?:long[- ]?term\s+)?holding|hold(?:ing)?\s+(?:long[- ]?term|for\s+the\s+long\s+term)|add|trim|sell|buy)\b/i,
  /\b(?:my|our)\s+position\s+in\s+\$?[a-z0-9.]{1,8}\b.{0,50}\b(?:worth\s+(?:long[- ]?term\s+)?holding|hold(?:ing)?\s+(?:long[- ]?term|for\s+the\s+long\s+term)|add|trim|sell|buy)\b/i,
  /\b(?:my|our)\s+holdings?\b.{0,50}\b(?:worth\s+(?:long[- ]?term\s+)?holding|hold(?:ing)?\s+(?:long[- ]?term|for\s+the\s+long\s+term)|add|trim|sell|buy)\b/i,
  /\b[Ii]s\s+\$?[A-Z]{1,6}(?:\.[A-Z]{1,3})?\s+(?:worth\s+(?:long[- ]?term\s+)?holding|hold(?:ing)?\s+(?:long[- ]?term|for\s+the\s+long\s+term)|a\s+(?:long[- ]?term\s+)?hold)\b/,
  /\b(?:stock|shares?|holdings?|portfolio)\b.{0,50}\b(?:worth\s+(?:long[- ]?term\s+)?holding|hold(?:ing)?\s+(?:long[- ]?term|for\s+the\s+long\s+term)|add|trim|sell|buy)\b/i,
  /\b(?:worth\s+(?:long[- ]?term\s+)?holding|hold(?:ing)?\s+(?:long[- ]?term|for\s+the\s+long\s+term)|add|trim|sell|buy)\b.{0,50}\b(?:stock|shares?|holdings?|portfolio)\b/i,
];
