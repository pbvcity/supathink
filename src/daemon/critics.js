...
//  fullReview —— full 档单次全审(deepseek/gpt54mini 复用 lib/critic;haiku 走订阅需异步化防塞 daemon 事件循环)
...
async function fastCheck(draft, cfg, cache, budgetMs = FAST_BUDGET_MS) {
  ...
  // 与 claims-cache.claimHash 同构规范化:大小写无关 + 去空白标点,避免哈希命中了但文本比对因大小写漏报
  const norm = (s) => String(s || '').toLowerCase().replace(/[\s,,。;;::!!??、"'「」『』()()\[\]]+/g, '');
  const normDraft = norm(draft);
  const cachedHits = [];
  for (const e of cache.map.values()) {
    if (e.verdict === 'refuted' && e.claim_text && e.claim_text.length >= 12 && normDraft.includes(norm(e.claim_text))) {
  ...
  // 完全重复的 supported 断言本地放行,再交给 slowVerify 记录 cache_hit。
  // 这样 §4.2 的"缓存命中跳核验"不被快查模型因本地 fixture URL/格式噪声误拦。
  for (const e of cache.map.values()) {
    if (e.verdict !== 'supported' || !e.claim_text || e.claim_text.length < 20) continue;
    const n = norm(e.claim_text);
    if (n && normDraft.includes(n) && normDraft.length <= n.length + 30) {
      return { blockers: [], timed_out: false, ms: Date.now() - t0, usage: null, cache_supported: true };
    }
  }
...
async function slowVerify(draft, cfg, cache) {
...
async function fullReview(draft, cfg, opts = {}) {
  ...
  if (cfg.backend === 'deepseek' || cfg.backend === 'gpt54mini' || cfg.host === 'codex') {
    const { runCritic } = require(path.join(__dirname, '..', 'lib', 'critic'));
    return runCritic(draft, cfg, opts); // deepseek 异步;Codex 宿主回退 gpt-5.4-mini
  }
