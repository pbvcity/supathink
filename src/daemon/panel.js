...
async function runPanel(question, cfg) {
  ...
  const resolveSeat = (roleDef) => { // 主绑不可用时沿 fallback 链找可用引擎(kind:host 永不入席,§14.3)
    if (!roleDef || !roleDef.model) return resolveEngine('unknown', cfg);
  ...
  if (seats.length < 3) {
    return {
      ...
      absent: absent.map((s) => {
        const roleDef = caps.roles[s.role] || {};
        return `${s.role}=${roleDef.model || '未配置'}(引擎未配置/不可用)`;
      }),
    };
  }
  const judgeRole = caps.roles.judge || {};
  const judgeEng = resolveEngine(judgeRole.model || 'unknown', cfg);
  if (!judgeEng.available) return { error: `judge(${judgeRole.model || '未配置'})引擎不可用,panel 拒绝(无 Judge 不综合)`, absent: [] };
...
async function runDelphi(question, cfg) {
  ...
  const usage = r1.map((r) => r && r.usage).concat(r2.map((r) => r && r.usage), [sum && sum.usage]);
