...
const { loadConfig, detectForce, detectSlowFull, detectSessionToggle, detectPanel, detectAltitude } = require(path.join(__dirname, '..', 'lib', 'config'));
...
async function onUserPrompt(payload) {
  ...
  if (pbLib.RE_STRONG_APPROVE.test(payload.prompt || '')) {
    ...
    if (notable && !recentlyAsked) {
      ...
      saveState(sid, state);
      const parts0 = [`[超限思考] 检测到强认可。本次思考配方有特点(${r.mode}${r.panel ? '+panel' : ''}${r.intent ? '/' + r.intent : ''})。请询问用户:「要把这次的思考配方存为 playbook,下次同类命题自动复用吗?保存请回复:/st:playbook-save <名称> <范围:仅此话题|此类决策|所有业务策略>;或保持灵活不保存。」`];
