...
const { loadConfig, detectForce, detectSlowFull, detectSessionToggle, detectPanel, detectAltitude, detectWin } = require(path.join(__dirname, '..', 'lib', 'config'));
...
async function onUserPrompt(payload) {
  ...
  if (panelMode) {
    ...
    st.cwd = payload.cwd;
    saveState(sid, st);
    const question = String(payload.prompt || '').replace(/^\s*\/st[:\/]panel(-lite)?\b/, '').replace(/^\s*SUPATHINK_PANEL=(panel|lite)\b/, '').trim() || st.last_prompt || '(空议题)';
