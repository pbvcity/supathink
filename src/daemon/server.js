...
function launchPanelAsync(sid, turn, question, panelMode, cfg) {
  setImmediate(async () => {
    try {
      ...
      const r = await (RUNNERS[panelMode] || runPanel)(question, cfg);
...
// 模型自判升档(协议第 6 条):消费 cwd 域升档文件(10 分钟内有效,一次性)——判断归模型,执行归 hook
...
async function onUserPrompt(payload) {
  ...
  if (panelMode) {
    ...
    st.cwd = payload.cwd;
    saveState(sid, st);
    const question = String(payload.prompt || '').replace(/^\s*\/st[:\/-]?panel-?lite\b/, '').replace(/^\s*\/st[:\/-]?(panel|debate|delphi|redblue)\b/, '').replace(/^\s*SUPATHINK_PANEL=(panel|lite)\b/, '').trim() || st.last_prompt || '(空议题)';
  ...
  // /st:playbook-save:把上一轮配方存为 playbook(§11 认可即捕获的落笔步)
  ...
  if (pbSave) {
    ...
    const pb = pbLib.save(pbSave.name, pbSave.scope, recipe, recipe.source_prompt || st.last_prompt); // 关键词取决策轮原题,不是认可语
  ...
  state.ledger = ledgerLib.update(state.ledger, payload.prompt); // 目标账本随轮更新(§5.1,unconfirmed 起步)
  state.active_turns = (state.active_turns || 0) + 1; // §5.3 两轴素材
  ...
  // 认可即捕获(§11):强认可 ∧ 上轮配方有特点 ∧ 近期未弹过 → 提议存 playbook(trace user_feedback)
  if (pbLib.RE_STRONG_APPROVE.test(payload.prompt || '')) {
    appendTrace(sid, { turn: state.turn, type: 'user_feedback', signal: 'strong_approve', raw: String(payload.prompt).slice(0, 120) });
    const r = state.last_recipe;
    const notable = r && (r.mode === 'full' || r.panel || (r.triggers || []).some((t) => String(t).startsWith('user_')));
    const recentlyAsked = state.last_pb_suggest_turn && state.turn - state.last_pb_suggest_turn < 5;
    if (notable && !recentlyAsked) {
      state.last_pb_suggest_turn = state.turn;
      saveState(sid, state);
...
async function onStop(payload) {
  ...
  const t0 = Date.now();
  const sid = payload.session_id || 'unknown';
  const esc = consumeEscalation(payload.cwd); // 协议第 6 条:模型自判升档,优先于规则路由的结论
  if (!sessionStateExists(sid) && !esc) return {};
  const state = loadState(sid);
  ...
  if (payload.stop_hook_active && loops >= 2) {
    ...
    if (state.last_severe && state.last_severe.length) {
  ...
  const doBlock = (severeLines) => {
    ...
    if (!persisted && payload.stop_hook_active) { finalTrace({ critic_status: 'state_unwritable_loop_guard' }); return {}; }
  ...
  const [review, nav] = await Promise.all([
    ...
    navigator.run('full', state.last_prompt || '', draft, cfg, { ledger: state.ledger, staleGoal: (state.turns_since_goal_confirm || 0) >= 5 && !(state.ledger && state.ledger.terminal_goal && state.ledger.terminal_goal.confirmed) }),
  ]);
...
async function runSlowPath(sid, turn, draft, state, cfg, cache) {
  ...
  const [slow, nav] = await Promise.all([
    slowVerify(draft, state.last_prompt || '', cfg, cache),
    navigator.run('light', state.last_prompt || '', draft, cfg, { ledger: state.ledger, force, staleGoal: (state.turns_since_goal_confirm || 0) >= 5 && !(state.ledger && state.ledger.terminal_goal && state.ledger.terminal_goal.confirmed) }),
  ]);
...
async function onReview(payload) {
  ...
  if (toggle) {
    ...
    st.session_auto = toggle === 'on';
    saveState(sid, st);
    return { verdict: 'pass', session_auto: st.session_auto, instruction: `本会话超限思考已${toggle === 'on' ? '开启' : '关闭'},请告知用户。` };
  }
  if (force === 'off') return { verdict: 'pass', skipped: 'user_fast' };
  if (detectHelp(payload.prompt)) { // OpenClaw 聊天内 /st(菜单入口)→ 走 revise 通道让 agent 呈现清单(插件只对 revise 动作)
    return { verdict: 'revise', help: true, instruction: `用户在查询超限思考命令。请把下列命令清单原样呈现(在 OpenClaw/Telegram 里用无分隔符形态,如 /stslow /stpanel;打字输入 st:slow 亦可):\n${ST_COMMANDS.join('\n')}` };
  }
  const win = detectWin(payload.prompt); // §16 北极星:TG/飞书聊天内 /st:win
  if (win) {
    appendTrace(sid, { turn: 0, type: 'intercept_win', desc: win.desc });
    return { verdict: 'pass', win: true, instruction: `北极星已记录:「${win.desc}」。请向用户简短确认并感谢反馈,本条消息不需要其他回答。` };
  }
  const pm = detectPanel(payload.prompt); // TG/OpenClaw 端合议指令:/st:panel|debate|delphi|redblue
  if (pm) {
    const q = String(payload.prompt || '').replace(/^\s*\/st[:\/](panel-lite|panel|debate|delphi|redblue)\b/, '').replace(/^\s*SUPATHINK_PANEL=\S+/, '').trim();
    launchPanelAsync(sid, 0, q || '(空议题)', pm, cfg);
    return { verdict: 'pass', panel: pm, instruction: `${pm} 合议已在后台启动(多模型并行,约 1–3 分钟)。请先给出你自己的独立初步分析;综合结果会写入 ~/.supathink/sessions/${sid}/ 下的 panel-*.json,下一轮你可用 exec 读取该文件向用户呈现。` };
  }
  ...
  if (!effAuto && force !== 'full') {
    // 策略关且未强制:毫秒级放行;协议允许 agent 对高危话题带 self_judged:true 主动自查一次(skill 式)
    if (!payload.self_judged) return { verdict: 'pass', skipped: 'agent_policy_off', latency_ms: Date.now() - t0 };
  }

  if (!draft) return { error: 'draft 必填' };
  const decision = route(payload.prompt || '');
  // 插件通道每次交付都会调进来 → 必须尊重 off(P1:闲聊零打扰);agent 主动自查(self_judged/v1)才最低按 light
  ...
  const [review, nav] = await Promise.all([
    ...
    navigator.run(mode, payload.prompt || '', draft, cfg, {}),
  ]);
  ...
  if (!review) {
    ...
    return { mode, verdict: 'pass', degraded: true, annotations: [], nav_flags: nav.flags, note: 'Critic 调用失败,按 §3.4 放行' };
  }
