...
async function onPrecmd(payload) {
  ...
  if (detectHelp(prompt)) {
    ...
    const question = prompt.replace(/^\s*\/st[:\/-]?panel-?lite\b/, '').replace(/^\s*\/st[:\/-]?(panel|debate|delphi|redblue)\b/, '').replace(/^\s*SUPATHINK_PANEL=(panel|lite|debate|delphi|redblue)\b/, '').trim() || st.last_prompt || '(空议题)';
    appendTrace(sid, { turn: st.turn, type: 'route_decision', mode: 'off', stance_hint: null, triggers: [`panel_${panelMode}`], menu: [] });
    launchPanelAsync(sid, st.turn, question, panelMode, cfg);
    ...
    st.last_prompt = prompt.slice(0, 2000); // 普通轮记录原题,供 altitude/panel 回溯
  }
