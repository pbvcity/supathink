...
**AI 助手的慢思考副驾**——装在 Claude Code / Codex / OpenClaw(三宿主)之上,在回答交付给你之前完成两条正交轴的检查:

- **Critic(正确性轴)**:这对不对?——引用取回原文比对、事实检索核验、假 DOI/虚构期刊识别、自相矛盾与循环论证检查。可核验的一律交工具,不凭模型记忆。
- **Navigator(对齐轴)**:这是不是朝着你真正要的方向?——目标回溯、代理指标警报(你要的是流量还是能转化的流量?)、约束相容、交权信号的授权门。

发现严重问题就打回:主模型必须逐条回应批注(采纳/驳回给理由/待查),修订后带「── 核验 ──」脚注交付;仲裁不了的分歧如实呈现,不捏共识。

**为什么需要它**:AI 的两类失效——幻觉(答案是错的)和对齐失效(答案全对但优化错了目标)——恰好都发生在你最信任它的时刻。supathink 的唯一成功标准是:某一天,它拦下了一个你差点采纳的错误结论。
...
| 升档四通道 | **模型自判**(主通道,AI 理解语境自己升)· 你的命令(`/st:slow`)· 规则兜底网 · 项目/会话/agent 配置 |
| 思考方法系统 | 升档轮按意图注入方法菜单(9 族 69 法),模型自选并声明——**声明即契约**,full 档核验是否真用了 |
...
`auto:true` = 该 agent 自动分档;`auto:false` = 仅 `/st:slow`、`/st:on` 或高危话题自查可唤起;改完即生效无需重启;策略关的 agent 每轮仅一次本地回环(1–2ms)零成本。
...
**Your AI's reviewer.** supathink sits on top of Claude Code / Codex / OpenClaw and checks every answer on two orthogonal axes **before it reaches you**:

- **Critic (correctness)** — Are the citations real (fetched and compared against the source)? Do factual claims hold up (search-verified)? Any self-contradiction or circular reasoning? Everything verifiable goes to tools, never to the model's memory.
- **Navigator (alignment)** — Is this heading where *you* actually want to go? Goal trace-back, proxy-metric alerts ("do you want traffic, or traffic that converts?"), constraint checks, and a mandatory authorization gate whenever you say "just decide for me".

Serious findings send the draft back: the model must answer each annotation (accept / reject with reasons / defer), then deliver with a verification footer. Disagreements that can't be arbitrated are surfaced honestly — never papered over.

**Why**: AI fails in two ways — hallucination (the answer is wrong) and misalignment (the answer is right, for the wrong goal) — and both strike exactly when you trust it most. This project's single success metric: the day it intercepts a wrong conclusion you were about to act on.
...
`auto:false` agents can still be invoked via `/st:slow`, `/st:on`, or their own high-stakes self-check; edits apply instantly, no restart needed. Uninstall: `./uninstall.sh` (removes hooks/protocol/commands/plugin; keeps your data).
