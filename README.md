...
何时深思由谁定?默认由 **你显式开启**(`/st:slow`、`/st:on`、项目/用户 auto 配置);auto 开启后 AI 才可按语境自选方法并按需升档。
...
| 谁决定深思 | 默认只响应你的命令(`/st:slow`/`/st:on`)与项目/会话/agent 配置;auto 开启后模型可在该范围内自判选法/升档 |
...
| `/st:init` | AI 引导写配置 |
| CLI:`supathink status/log/stats/quota/win/on/off/escalate/panel…` | 状态/逐轮记录/四指标/成本/**标记一次真实救场(北极星)**/开关/手动升档/召集合议 |

#### 什么时候会发生什么

- **思考方法菜单**:本轮升到 light/full 时按意图从 9 族 69 法里注入 5–8 法(决策→矩阵/Pre-mortem/可逆性;估算→费米/基率;审稿→钢人/证伪…),模型自选并声明「本轮采用 X」——**声明即契约**,full 档核验是否真用了(违约=major 打回);off 轮不注方法菜单、不跑核验模型。
- **多模型合议**:**永不默认自动触发**(N× 成本)——只响应你的 `/st:panel|debate|delphi|redblue` 命令、auto 已开启范围内的模型自判召集、或终端 CLI;结果后台跑完或失败后于**下一轮作为强制可见交付事件**注入,要求先独立交付综合结论/失败状态再答新问题。若模型只解释规则/回答新问题而没独立交付,只记 attempted 并重新排队;逐条满足对应方法的交付模板后才记 delivered。唯一半自动:full 修订循环撞上限仍有分歧时,Judge 自动仲裁一次或如实标注未决,同样强交付。
- **谁决定升档**:默认由用户/配置决定;只有 `/st:on`、项目/用户 auto=true、OpenClaw agent auto=true 等显式开启后,模型才可在该范围内自判执行 `supathink escalate`。已升档轮次的核验与授权红线不因任何判断豁免。
...
`auto:true` = 该 agent 自动分档;`auto:false` = 仅手动命令(插件形态 `/st:slow` `/st:on`,OpenClaw/TG 菜单形态 `/stslow` `/ston`)可唤起;改完即生效无需重启;策略关的 agent 每轮仅一次本地回环(1–2ms)零成本。
...
Who decides when to think hard? By default, **you do**: explicit commands (`/st:slow`, `/st:on`) or project/user auto config. Once auto is enabled, the AI may pick methods and escalate within that scope.
...
| Who escalates | Your commands (`/st:slow`, `/st:on`) and per-project/session/agent config by default; model self-escalation is only allowed after auto has been explicitly enabled |
...
| `/st:panel` · `/st:panel-lite` · `/st:debate` · `/st:delphi` · `/st:redblue` | deliberation workflows (run in background; result returns next turn as a required visible delivery before new answers) |
...
`auto:false` agents can still be invoked manually (plugin form `/st:slow` `/st:on`, OpenClaw/TG menu form `/stslow` `/ston`); edits apply instantly, no restart needed. Uninstall: `./uninstall.sh` (removes hooks/protocol/commands/plugin; keeps your data).
