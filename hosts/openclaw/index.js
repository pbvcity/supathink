...
export default function register(api) {
  ...
  api.on('before_prompt_build', async (event, ctx) => {
    try {
      ...
      if (precmdOk.size > 200) for (const [key, ts] of precmdOk) if (Date.now() - ts > 120_000) precmdOk.delete(key); // 有界:清 finalize 未消费的陈旧标记
  ...
  log('registered: before_prompt_build → /v1/hook/precmd; before_agent_finalize → /v1/review');
}
