...
(async () => {
  ...
  process.stdout.write('  等缓存基线慢查(≤180s)…');
  let hitTrace = [];
  for (let w = 0; w < 180; w++) { await sleep(1000); hitTrace = readTrace(sidHit); if (hitTrace.some((e) => e.type === 'verification_result')) break; }
  console.log('');
  ...
  process.stdout.write('  等第二轮缓存命中(≤180s)…');
  let hit = false;
  for (let w = 0; w < 180; w++) {
    await sleep(1000);
