...
const TYPES = ['route_decision', 'draft_captured', 'claims_extracted', 'verification_result',
  ...
  'playbook_capture', 'escalation_ignored', 'intercept_win'];
...
function runSelfTest() {
  ...
  if (goodErrs.length || badErrs.length < 2) {
    console.log(`✗ schema 自检失败 good=${goodErrs.join(';') || 'ok'} bad=${badErrs.join(';') || 'missing'}`);
    return false;
  }
  return true;
}
