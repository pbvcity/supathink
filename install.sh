...
$ARGUMENTS
EOF
for c in debate delphi redblue slow-full win; do
  ...
  case "$c" in
    ...
    win) MARKER="SUPATHINK_WIN=1"; DESC="$DESC:北极星——记录一次真实救场(系统拦下了你差点采纳的错误结论)";;
  esac
...
EOF
  ...
  else echo "Codex 命令由插件壳提供,跳过 prompts"; fi
else
