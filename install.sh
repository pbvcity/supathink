...
# —— /st:* 用户命令(命名空间目录;由插件壳驱动安装时跳过,避免与插件自带命令重复)——
if [ "${SUPATHINK_SKIP_CC_COMMANDS:-0}" = "1" ]; then
  echo "命令由插件壳提供,跳过写入 ~/.claude/commands"
else
...
# 单源:命令定义只活在 plugins/st/commands/(壳插件与直装共用),init 除外
cp "$SRC"/plugins/st/commands/*.md "$CMDS/st/"
# init 用"已安装态"的配置向导覆盖(壳里的 init 是安装引导,语境不同)
...
# —— CC supathink skill(手动/auto 已启用时的入口;插件壳驱动时跳过)——
if [ "${SUPATHINK_SKIP_CC_COMMANDS:-0}" != "1" ]; then
...
---
默认不要自行升级本轮思考;只有用户显式命令或 auto 已开启时才使用下列入口:
...
3. 北极星代记:用户明确说某次拦截救了他(「幸好拦住了」「差点就信了」)时,Bash 运行 `supathink win "<引用其原话>+<场景>"` 并告知;泛泛夸奖不算,拿不准先问。
4. 红线:已升档轮次的核验与授权之门,不因你的判断而豁免。
EOF
echo "supathink skill 已安装:~/.claude/skills/supathink"
fi

# —— Codex 宿主接线(Phase 3;三宿主原则的一支)——
...
EOF
  ...
  # Codex 命令:装了 st 插件(codex-skills → /st:)就跳过 flat prompts;否则写 flat(/st- fallback)
  if codex plugin list 2>/dev/null | grep -q "st@supathink.*installed"; then
    echo "Codex 已装 st 插件(/st: 命令),跳过 flat prompts"
  else
  mkdir -p "$CODEX_HOME_DIR/prompts"
  cp "$SRC"/plugins/st/commands/init.md "$CODEX_HOME_DIR/prompts/st-init.md"
  for c in slow slow-full fast on off panel panel-lite debate delphi redblue altitude win; do
    ...
    case "$c" in
      ...
      win) MARKER="SUPATHINK_WIN=1";;
    esac
    printf '%s\n\n$ARGUMENTS\n' "$MARKER" > "$CODEX_HOME_DIR/prompts/st-$c.md"
  done
  echo "Codex prompts 已安装:/st-slow /st-fast /st-on /st-off /st-init /st-panel /st-panel-lite /st-altitude 等"
  fi
else
...
fs.writeFileSync(process.argv[3], JSON.stringify({ _注释: 'OpenClaw agent 级策略:auto=true 的 agent 按需分档;false 只响应 /st:slow、/st:on 等显式命令', default: 'off', agents }, null, 2) + '\n');
